...
async def _option_quote(streamer: DataStreamer, option_list: List[List[Option]]) -> List:
    # Master list of every option code in the chain
    res = []
...
def option_quote(streamer: DataStreamer, option_list: Union[List[List[Option]], OptionChain]) -> List:
    #Create temporary async loop to execute op
    loop = asyncio.get_event_loop()
    resp = loop.run_until_complete(_option_quote(streamer,option_list))
    return resp